...
- `starforge:stage` 怪潮脚本条件（2026-09-25 新增）：The Hordes 上游的 `gamestages:gamestage` 条件绑定 darkhax GameStages（未安装），本条件经 `DataRegistry.registerConditionDeserializer` 公开钩子注册、镜像其 JSON 形式（`{"name":"starforge:stage","value":"<stage id>"}`，value 走 ValueGetter 可接动态值），后端为 `ProgressiveStagesAPI.hasStage(player, StageId)` ——team scope 时代阶段按目标玩家所在团队解析，FakePlayer 兼容。无 Mixin、无轮询；仅在 hordes + progressivestages 双装时注册。hordetest 实测：同一 FakePlayer 依 grant 档位从 `HordeBuildSpawnDataEvent` 真实取表 `hordes:default → default_t2 → default_t3 → default_t5 → default_t6` 逐级切换，无阶段保持 `hordes:default`。
- FastPipes capability 互通实测（2026-09-24，starforge_captest.js）：物品管抽取附件向箱子真实交付；流体管在 BC 储罐间搬 16000 mB 水无损耗；能量管从 Ad Astra energizer 抽 FE 送入 IC2 电炉（源净失≥目的净增+管网缓存，无增殖）。IC2CRE batbox/发电机/电缆全六面暴露 EnergyStorage——原生 FE 桥确认，无需另做转换器；Railcraft charge_terminal/charge_motor 不暴露 FE——并网由 `starforge_compat:charge_bridge` 承担（官方 ChargeBlock 节点 API，captest 实测 FE→Charge 与 Charge→FE 均严格 1:1、256 FE/t、FastPipes 双向端到端守恒、红石抑制与 AUTO 滞回正常）；BC mj_dynamo 顶面出 FE（engine_fe 系物品 id、探测打到空气，当时"不暴露 cap"系假象）；AE2 接收器/控制器全六面收 FE；Refurbished cooler 无物品 cap、厨房水槽流体桥实测六面可读。2026-09-24 更新：BC `powerMode` 切到 `DISPLAY_FE`（`pack/config/buildcraftcore-common.toml`）后实测——`pump/quarry/laser` 全六面暴露收电 FE cap；`buildcraftcore:engine[type=stone|iron]` 无 FE cap（推送侧走 MjPort→FeEndpoint）；`engine[type=fe]` 全六面收 FE（此前 engine_fe 行实测为探测物品 id 打到空气，已修正）；能量管 → `buildcraftfactory:pump` 端到端入账 1000 FE 守恒成立；燃煤红石点火的 `engine[type=stone]` → 能量管 → `ic2cre:electric_furnace` 实测管道缓存 770、炉收 1200——BC 引擎直推 FastPipes 能量管、再供任意 FE 设备，全程无 mj_dynamo 参与。2026-10-16 更新（Ad Astra 发电并网实测）：`coal_generator`/`solar_panel` 全六面+`any` 暴露 `EnergyStorage` cap（canExtract/canReceive 均 true）；`coal_generator`（煤入槽位 1）→ 能量管+抽取附件 → `ad_astra:compressor` 实测 200 tick 入账 3920 FE（≈20 FE/t，源/管网/目的端守恒、无增殖）；`solar_panel` 跨维度实测：主世界（solar_power 16）→ IC2 电炉收 1200 FE、月球（24）→ AA 压缩机收 1848 FE、地球轨道（32）→ AE2 控制器收 860–1872 FE（AE2 侧缓冲自身有回流波动）；三维度 `dayTime%24000` 共享同一世界时钟（`execute in <dim> time query daytime` 逐维实测同步递增），`isDay` 在 12000 边界统一翻转——Ad Astra 自定义天空渲染与世界时钟同源，未发现「视觉白天/逻辑夜晚」错配；月球「不发电」复现结果：面板被置于天空遮挡之下（canSeeSky=false）时才不发，正常地表放置发电正常，故未添加兼容修复。
