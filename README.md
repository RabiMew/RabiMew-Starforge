...
当前状态：**dedicated server 可在 Java 21 + NeoForge 21.1.251 上启动到 `Done`，88 个服务端模组零错误加载**（清单全量 111 个启用条目，其中 109 个进客户端；locked-mods 由 manifest 自动生成）；材料统合层（Almost Unified + KubeJS `c:` 标签层）已让重复金属输出全部收敛到标准物品，AE2↔厨房双桥（Applied Cooking / Applied Delight）已装载；自有附属 **Starforge Compatibility** 已交付红石怪潮警报（`horde_alarm`）、厨房动态热源（`electric_burner`）、Refurbished 电脑双应用（Starforge Control / Security）与流体容器 `IFluidHandler` 桥；T0–T7 八阶段 ProgressiveStages 推进链（真实合成触发 + 计数器后备）已在运行时验证；SF-01..34 联动配方、统一流体桥（原油 8 种/燃料 22 种/杂酚油 6 种互通）、P2 防御配置层（Hordes/In Control/ZBB/TaCZ 守卫）已就位；T1 火箭地球闭环经配方图验证。装饰层由 Supplementaries + **Macaw's Furniture** + **MrCrayfish's Furniture Mod: Refurbished**（含 Framework 前置，电脑/电力家电体系原生接入 CFB 厨房网络）构成——家具定位为基地、工业设施、空间站与殖民地的生活/装饰层，消耗统一木材/铁/玻璃材料，不建立独立科技树；能源层以 FE 公共电网为核心：IE/Ad Astra 直接产 FE，BuildCraft 经原生 FE↔MJ，Refurbished 家庭电网由 **Energized Furniture** 变压器接入，IC2CRE 保留独立 mEU 体系；食品工业链为 Farmer's Delight → CFB/Refurbished 现代厨房 → IE + **Engineers Delight** 工业加工 → **Immersive Cooking & Farming** 自动化 → AE2 → 殖民/航天后勤。FramedBlocks 与 Building Gadgets 2 等建筑工具保留。实测明细见 [实施状态](docs/implementation-status.md)。
