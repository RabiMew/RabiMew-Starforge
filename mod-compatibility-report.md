...
- No worldspike/chunk-loading toggle exists in `railcraft-server.toml` — the worldspike family is instead held at T4-tier cost by its KubeJS recipe (steel components + advanced circuits); `personal_world_spike` stays cheap since it only loads chunks while the owner is online.
...
- **Tier placement** (`tech-tiers.json`, affordability metadata — no runtime locks): `electric_age` lists `ef_transformer`/`rf_computer`/`tmted_knife` (`rf_generator` was later removed with the generator disable list); `heavy_industry_age` lists `ic_cookpot`/`ic_fermenter`/`ic_grill`/`ic_processor`. Furniture decor itself stays T0 (cheap building materials).
...
**2026-10-16 — Ad Astra generators restored to the grid**: `coal_generator`/`solar_panel` re-enabled (upstream recipes/EMI/JEI/creative restored; only Refurbished generators + AE2 vibration chamber stay disabled). Both expose `EnergyStorage` on all six sides + `any` (extract/receive both true; internal caps 10,000 / 50,000 FE) — native grid members, no bridge added. Ticked FastPipes rigs: coal generator (coal into fuel slot 1) → energy pipe + extractor → `ad_astra:compressor` delivered 3,920 FE over a 200-tick window (≈20 FE/t, matching `coalGeneratorEnergyGenerationPerTick=20`), conserved; solar panel → pipe → IC2 electric furnace in overworld (solar_power 16) delivered 1,200 FE inside a 100-tick day window; the same rig on `ad_astra:moon` (solar_power 24) delivered 1,848 FE to an AA compressor, and on `ad_astra:earth_orbit` (32) fed an AE2 controller. Cross-dimension sampling (t60/t150/t200) shows `getDayTime()` shares one world clock in every dimension, `isDay()` flips at `dayTime%24000 = 12000` uniformly, and `canSeeSky` gating works — no custom-sky/vanilla-clock mismatch was observed; a moon panel that "doesn't generate" was reproduced only by burying it out of sky sight. Per-planet solar_power values are unchanged.
...
## Addendum — 2026-09-25 version bump + horde stage tables (114 enabled)

**Version bumps** (resolved via `fetch-mods --only`, re-locked, `audit-deps` clean on 114 jars): Sophisticated Backpacks 3.25.78.2107 → 3.26.3.2158 · Balm 21.0.65 → 21.0.66 · Moonlight Lib 3.6.8 → 3.6.9 · AzureLib 3.1.11 → 3.1.12. Dedicated server `Done (1.965s)`, zero new errors. (Windows-only hiccup seen once: `taczpackupgrader` crashed in mod construction with `AccessDeniedException` when a leftover `temp/` unpack dir existed — deleting `run/server/taczpackupgrader/temp` fixed it; transient file lock, unrelated to the bumps.)

**Horde stage composition** — the long-standing `gamestages:gamestage` gap is closed in `starforge_compat`: new `HordeStageCondition` registers `starforge:stage` through The Hordes' public `DataRegistry.registerConditionDeserializer` hook (no mixin), same JSON shape as upstream (`{"name":"starforge:stage","value":"<stage id>"}`, value via `ValueGetter`), backend `ProgressiveStagesAPI.hasStage` — team-scope era stages resolve against the target player's team. Registered only when `hordes` + `progressivestages` are both loaded.

`horde_data/tables/` split into five era-band tables (`default` T0–T1 melee / `default_t2` electric_age +ranged & spider / `default_t3` information_age covers T3–T4 +creeper-stray-bogged-witch / `default_t5` atomic_age +mutant_zombie & zombie-horse rider / `default_t6` space_age covers T6–T7 +skeleton-horse archer, cave-spider jockey, phantom-carried creeper); `scripts/default.json` applies them in ascending-stage order (last match wins — linear stage chain means the highest owned tier applies); the ocean-biome drowned override stays last. `first_day` windows only remain on rare mounted/elite entries as intra-tier floors.

**Runtime evidence**: KubeJS `starforge_horde_stage_test.js` (flag `kubejs/hordetest.json`) — FakePlayer + real `HordeEvent` from `HordeSavedData`, a real `HordeBuildSpawnDataEvent` evaluated through `HordeScriptLoader.applyScripts`: 7/7 table selections correct (none/mechanical → `hordes:default`, electric → `default_t2`, information → `default_t3`, atomic → `default_t5`, space → `default_t6`). Per-planet T6 ecology variants still pending — The Hordes has no dimension condition/ValueGetter; a `starforge:dimension` condition in the same registry slot is the reserved path.
